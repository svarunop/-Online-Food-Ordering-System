...
const SuperAdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:7000/api/super-admin/notifications')
  ...
  const sendNotification = async () => {
    if (!newNotification.trim() || !discountPercentage.trim() || !validityHours.trim()) return;
    try {
      await axios.post('http://localhost:7000/api/super-admin/send-promo-notification', {
        user_id: 1, // Replace with dynamic user_id
        message: newNotification,
        discount_percentage: discountPercentage,
        validity_hours: validityHours,
      });
      setNotifications([{ message: newNotification, timestamp: new Date().toLocaleString() }, ...notifications]);
      setNewNotification('');
                  ...
                  <Col md={3}>
                    <Card className="p-3">
                      ...
                      <Form.Group>
                        <Form.Control
                          type="text"
                          placeholder="Enter promo code or offer"
